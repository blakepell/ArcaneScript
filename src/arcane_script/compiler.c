...
// used to restore compiler's state if something fails
...
static const symbol_t* define_symbol(compiler_t* comp, src_pos_t pos, const char* name, bool assignable,
                                     bool can_shadow);

compiler_t* compiler_make(allocator_t* alloc, const arcane_config_t* config, gcmem_t* mem, errors_t* errors,
                          ptrarray(compiled_file_t)* files, global_store_t* global_store)
{
...
static bool import_module(compiler_t* comp, const statement_t* import_stmt)
{
	...
	for (int i = 0; i < ptrarray_count(file_scope->loaded_module_names); i++)
	{
		...
		{
			errors_add_errorf(comp->errors, ERROR_COMPILATION, import_stmt->pos, "Module \"%s\" was already imported",
			                  module_name);
			result = false;
			goto end;
		}
	...
	{
		errors_add_error(comp->errors, ERROR_COMPILATION, import_stmt->pos,
	...
	for (int i = 0; i < ptrarray_count(comp->file_scopes); i++)
	{
		...
		{
			errors_add_errorf(comp->errors, ERROR_COMPILATION, import_stmt->pos, "Cyclic reference of file \"%s\"",
			                  filepath);
			result = false;
			goto end;
		}
	...
	if (!module)
	{
		...
		if (!code)
		{
			errors_add_errorf(comp->errors, ERROR_COMPILATION, import_stmt->pos, "Reading module file \"%s\" failed",
			                  filepath);
			result = false;
			goto end;
		}
...
static bool compile_statement(compiler_t* comp, const statement_t* stmt)
{
	...
	switch (stmt->type)
	{
		...
		case STATEMENT_DEFINE:
		{
			...
			const symbol_t* symbol = define_symbol(comp, stmt->define.name->pos, stmt->define.name->value,
			                                       stmt->define.assignable, false);
			if (!symbol)
			{
				return false;
			}
		...
		case STATEMENT_FOREACH:
		{
			...
			const symbol_t* iter_symbol = define_symbol(comp, foreach->iterator->pos, foreach->iterator->value, false,
			                                            false);
			if (!iter_symbol)
			{
				return false;
		...
		case STATEMENT_RECOVER:
		{
			...
			const symbol_t* error_symbol = define_symbol(comp, recover->error_ident->pos, recover->error_ident->value,
			                                             false, false);
			if (!error_symbol)
			{
				return false;
...
static bool module_add_symbol(module_t* module, const symbol_t* symbol)
{
	...
	symbol_t* module_symbol = symbol_make(module->alloc, strbuf_get_string(name_buf), SYMBOL_MODULE_GLOBAL,
	                                      symbol->index, false);
	strbuf_destroy(name_buf);
	if (!module_symbol)
	{
